...
async function waitForExit(pids: number[], timeoutMs: number): Promise<number[]> {
  ...
  for (const pid of pids) {
    ...
    const r = Bun.spawnSync(["kill", "-0", String(pid)]);
    if (r.exitCode === 0) remaining.push(pid);
  }
  return remaining;
}

export async function runReload(args: string[]): Promise<void> {
  console.log(`agents-office reload v${VERSION}`);
  console.log("");
  ...
  if (!onlyDaemon) {
    ...
    if (ccPids.length > 0) {
      ...
      if (stuck.length > 0) {
        console.log(`  \u26A0  ${stuck.length} Claude Code process(es) did not exit — send SIGKILL?`);
        for (const pid of stuck) {
          console.log(`       run: kill -9 ${pid}`);
        }
      } else {
        console.log(`  \u2713 Claude Code exited gracefully`);
      }
      actions.push("claude-code");
    } else {
      console.log(`  \u2014 Claude Code not running`);
    }

    // Phase 1b: Graceful SIGINT to OC
    ...
    if (ocPids.length > 0) {
      ...
      if (stuck.length > 0) {
        console.log(`  \u26A0  ${stuck.length} OpenCode process(es) did not exit`);
      } else {
        console.log(`  \u2713 OpenCode exited gracefully`);
      }
      actions.push("opencode");
    } else {
      console.log(`  \u2014 OpenCode not running`);
    }
  }
  ...
  if (!onlyAgents) {
    if (findDaemonPidInSystemd()) {
      ...
      const r = Bun.spawnSync(["sudo", "systemctl", "restart", "agents-office"]);
      if (r.exitCode === 0) {
        console.log(`  \u2713 daemon restarted (systemd)`);
      } else {
        console.log(`  \u2717 daemon restart failed (systemd)`);
        warnings.push("daemon-restart");
      }
      actions.push("daemon");
    } else {
      ...
      if (agentPids.length > 0) {
        ...
        const stillRunning = findPids("agents-office");
        if (stillRunning.length === 0) {
          // HUP killed it, need to restart
          console.log(`  \u26A0 daemon exited on HUP — re-start it manually`);
          warnings.push("daemon-restart");
        } else {
  ...
  // Summary
  console.log("");
  if (actions.length === 0 && !onlyDaemon && !onlyAgents) {
    console.log(`  nothing to reload`);
    return;
  }

  const parts = actions.map((a) => `\u2713 ${a}`);
  console.log(`  reloaded: ${parts.join(", ")}`);
